...
(function () {
    ...
    Class.extend = Array.extend = function (prop) {
		
		if (arguments.length > 1) {
			var _ = require("lib/underscore");
			prop = _.extend.apply(_, arguments);
		}
		...
		var override = function(name, fn, source, target) {
			if (typeof(fn) == "function" && typeof(source[name]) == "function" && fnTest.test(fn)) {
				...
				Ti.API.info('Overriding name by value: ' + name);
				target[name] = fn;
			}
		}
        ...
        for (var name in _super) {
            ...
            if ((g && !og) || (s && !os)) {
                if (g)
                	Ti.API.info('Defining Getter on Prototype: ' + name);
