...
(function() {
	...
	$.Context = Class.extend({
		init : function(options) {
			...
			this.data = (this.options.data && this.options.data instanceof $.ReaderData) ? 
							this.options.data : new $.ReaderData({
								raw : this.options.data
							});
			this.data.context = this;

			this.id = _.uniqueId("_build_context");
			ctx[this.id] = { context : this};
		...
		createChild : function(options) {
			...
			var c = new $.Context(_.extend({}, this.options, {data : null}, options, {parent : this}));
			isProxy ? (this.proxyChild = c) : this.childs.push(c);
			
			return c;
		},
		addBuild: function(fn, instance) {
			if (!this.__root) this.__root = this.root();
			if (!this.__buildchain) this.__buildchain = $.Context.createBuildChain(this);
		...
		createInstance : function(rebuild) {
			...
			if (this.reader && (!this.__hasBuild || rebuild == true)) this.reader.execute();
		...
		return {
			add: function(fn, instance) {
				...
				data.push(function(ptr) {
					var tSetter = context.setInstance, 
						tGetter = context.getInstance,
						tPtr = context.ptr,
						pointer = ptr;
